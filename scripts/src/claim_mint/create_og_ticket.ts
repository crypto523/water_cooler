...
(async () => {
        ...
        console.log("User1 creates OriginalGangsterTicket");

        txb.moveCall({
            target: `${packageId}::mint::create_og_ticket`,
            arguments: [
                txb.object(mintcap),
    ...
    const ticket = `${packageId}::mint::OriginalGangsterTicket`;
    const ticket_id = find_one_by_type(objectChanges, ticket);

    if (!ticket_id) {
        console.log("Error: Could not find OriginalGangsterTicket object");
        process.exit(1);
    }
    userObjects.user_objects.OriginalGangsterTicket = ticket_id;
